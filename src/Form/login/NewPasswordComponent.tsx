    ...
    if(passwords.isValid && !!passwords.password1 && !!passwords.password2) {
      ...
      try {
        const response = await fetch(
          "http://localhost:3001/user/user-password-reset",
          {
            method: "POST",
            credentials: "include",
            headers: {
  ...
  return (
    <div className="mb-14">
      <div
        ...
        } grid grid-cols-1 place-items-center text-white text-rose-600 z-10 `}
      >
      ...
      <div className="mt-10 mx-3 shadow-2xl shadow-blue-200 sm:mx-14 md:mx-16 lg:mx-18 xl:mx-21.5 rounded-2xl">
        <div className="mt-10 pt-3.5 pl-3.5 font-semibold text-center">Reset your password</div>
        <div
          className={`${
            processes.state === "error-response" ? "pt-3.5" : ""
          } text-center font-semibold text-rose-600`}
        >
          {processes.response}
        </div>

        <div className="text-sm text-rose-400 py-9 px-0.5 sm:px-3.5 text-center">
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 gap-3 place-items-center">
            <div className="mb-9">
              <input
                ref={passwordRef}
                className="placeholder-place-col w-21 max-[380px]:w-18 h-9 focus:outline-none border-blue-600 border-2 rounded-4xl pl-2"
            ...
            <div className="mb-9">
              <input
                className="placeholder-place-col w-18 max-[380px]:w-16.5 h-9 inline mr-2 focus:outline-none border-blue-600 border-2 rounded-4xl pl-2"
                type={typeChange ? "text" : "password"}
